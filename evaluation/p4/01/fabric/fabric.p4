...
// @pi4("(FabricParser) as (x:{y: meta . standard_metadata | y.pkt_in.length > 687}) -> ethernet~")
// @pi4("(FabricDeparser) as (x:{y: meta . standard_metadata . ethernet | y.pkt_in.length == 0 ∧ y.pkt_out.length == 0}) -> {y:⊤| y.pkt_out.length > 0}")
@pi4("(FabricParser;FabricIngress;FabricDeparser) as (x:{y: meta . standard_metadata | y.pkt_in.length > 687 && y.pkt_out.length == 0}) -> ⊤")

...
let test_chomp_t1_length_in_changed1 () =
  let t = Length (0, PktIn) in
  let actual = T.chomp_t1 t 0 0 in
  let expected = Plus (Length (0, PktIn), Num 1) in
  Alcotest.(check term)
...
let test_chomp_t1_concat_nil () =
  let t = Concat (Bv Nil, Bv Nil) in
  let actual = T.chomp_t1 t 0 0 in
  let expected = t in
  Alcotest.(check term) "chomp_t1 <>@<> should be <>@<>" expected actual

let test_chomp_t1_concat_inductive_left () =
  let t = Concat (Slice (Packet (0, PktIn), 11, 21), Bv (Cons (One, Nil))) in
  let actual = T.chomp_t1 t 0 0 in
  let expected =
    Concat (Slice (Packet (0, PktIn), 10, 20), Bv (Cons (One, Nil)))
  in
  Alcotest.(check term)
    "chomp_t1 x.PktIn[11:21]@<> should be x.PktIn[10:20]@<>" expected actual

let test_chomp_t1_concat_inductive_right () =
  let t = Concat (Bv (Cons (One, Nil)), Slice (Packet (0, PktIn), 0, 21)) in
  let actual = T.chomp_t1 t 0 3 in
  let expected =
    Concat
      ( Bv (Cons (One, Nil)),
...
let test_chomp_t1_slice_inst1 () =
  let t = Slice (Instance (0, foo_inst), 0, 10) in
  let actual = T.chomp_t1 t 0 0 in
  let expected = t in
  Alcotest.(check term)
...
let test_chomp_t1_slice_inst2 () =
  let t = Slice (Instance (1, foo_inst), 123, 456) in
  let actual = T.chomp_t1 t 0 0 in
  let expected = t in
  Alcotest.(check term)
...
let test_chomp_t1_slice_out1 () =
  let t = Slice (Packet (0, PktOut), 123, 456) in
  let actual = T.chomp_t1 t 1 0 in
  let expected = t in
  Alcotest.(check term)
...
let test_chomp_t1_slice_in_unchanged () =
  let t = Slice (Packet (0, PktIn), 123, 456) in
  let actual = T.chomp_t1 t 1 0 in
  let expected = t in
  Alcotest.(check term)
...
let test_chomp_t1_slice_in_shift_indices () =
  let t = Slice (Packet (0, PktIn), 123, 456) in
  let actual = T.chomp_t1 t 0 0 in
  let expected = Slice (Packet (0, PktIn), 122, 455) in
  Alcotest.(check term)
    "chomp_t1 x.PktIn[123:456] x should be x.PktIn[122:455]" expected actual

let test_chomp_t1_slice_in_r_is_1 () =
  let t = Slice (Packet (0, PktIn), 0, 1) in
  let actual = T.chomp_t1 t 0 0 in
  let expected = Bv (Cons (B 0, Nil)) in
  Alcotest.(check term)
    "chomp_t1 x.PktIn[0:1] x B0 should be B0 :: <>" expected actual

let test_chomp_t1_slice_in_r_is_not_1 () =
  let t = Slice (Packet (0, PktIn), 0, 21) in
  let actual = T.chomp_t1 t 0 7 in
  let expected =
    Concat (Bv (Cons (B 7, Nil)), Slice (Packet (0, PktIn), 0, 20))
  in
  Alcotest.(check term)
...
let test_heapref_concrete_bv () =
  let t = bv_s "0101101000001111" in
  let actual = T.heap_tref1 t 0 0 foo_inst 0 in
  let expected = t in
  Alcotest.(check term) "heap_tref1 <0101101000001111>" expected actual

let test_heapref_b0 () =
  let t = Bv (Cons (B 0, Nil)) in
  let x = 1 in
  let actual = T.heap_tref1 t 0 x foo_inst 28 in
  let expected = Slice (Instance (x, foo_inst), 0, 1) in
  Alcotest.(check term) "heap_tref1 <b0>" expected actual

let test_heapref_b1 () =
  let t = Bv (Cons (B 1, Nil)) in
  let x = 1 in
  let actual = T.heap_tref1 t 0 x foo_inst 28 in
  let expected = t in
...
let test_heapref_b1b0 () =
  let t = bv_l [ B 1; B 0 ] in
  let x = 1 in
  let actual = T.heap_tref1 t 0 x foo_inst 28 in
  let expected =
    Concat (Bv (Cons (B 1, Nil)), Slice (Instance (x, foo_inst), 0, 1))
  in
  Alcotest.(check term) "heap_tref1 <b1, b0>" expected actual

let test_heapref_concretebv_b0 () =
  let t = bv_l [ Zero; Zero; One; One; B 0 ] in
  let x = 1 in
  let actual = T.heap_tref1 t 0 x foo_inst 28 in
  let expected =
    Concat (bv_l [ Zero; Zero; One; One ], Slice (Instance (x, foo_inst), 0, 1))
  in
  Alcotest.(check term) "heap_tref1 <0011 b0>" expected actual

let test_heapref_b0b1 () =
  let t = bv_l [ B 0; B 1 ] in
  let x = 1 in
  let actual = T.heap_tref1 t 0 x foo_inst 28 in
  let expected =
    Concat (Slice (Instance (x, foo_inst), 0, 1), Bv (Cons (B 1, Nil)))
  in
  Alcotest.(check term) "heap_tref1 <b0 b1>" expected actual

let test_heapref_b0_concretebv () =
  let t = bv_l [ B 0; Zero; One; Zero; One ] in
  let x = 1 in
  let actual = T.heap_tref1 t 0 x foo_inst 28 in
  let expected =
    Concat (Slice (Instance (x, foo_inst), 0, 1), bv_l [ Zero; One; Zero; One ])
  in
  Alcotest.(check term) "heap_tref1 <b0 0101>" expected actual

let test_heapref_b0b0 () =
  let t = bv_l [ B 0; B 0 ] in
  let x = 1 in
  let actual = T.heap_tref1 t 0 x foo_inst 28 in
  let expected =
    Concat
...
let test_reduce_concat2 () =
  ...
  (* let expected = Concat (bv_s "00", Concat (Bv (Cons (One, Nil)), Concat (Bv
     (Cons (One, Nil)), Slice (Instance (0, foo_inst), 0, 1)))) in *)
  let expected = Slice (Instance (0, foo_inst), 0, 5) in
  Alcotest.(check term) "reduce concat" expected actual
